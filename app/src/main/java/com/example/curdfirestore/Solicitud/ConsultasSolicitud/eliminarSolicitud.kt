...
fun eliminarSolicitudPorHorarioId(horarioId: String) {
    try {
        val db = FirebaseFirestore.getInstance()
        val solicitudRef = db.collection("solicitud")
        solicitudRef.whereEqualTo("horario_id", horarioId)
            .get()
            .addOnSuccessListener { documents ->
                for (document in documents) {
                    document.reference.delete().addOnCompleteListener { task ->
                        if (task.isSuccessful) {
                            println("Documento con horario_id $horarioId eliminado correctamente de Firestore.")
                        } else {
